...
async function registerYeongamProducts() {
  try {
    console.log('Registering 영암몰 products...');
    
    // Read scraped products
    const scrapedProducts: Product[] = JSON.parse(
      readFileSync('./scripts/output/yeongam-products.json', 'utf-8')
    );
    ...
    writeFileSync('./scripts/output/yeongam-registration-summary.json', JSON.stringify(summary, null, 2));
...
registerYeongamProducts().then(summary => {
