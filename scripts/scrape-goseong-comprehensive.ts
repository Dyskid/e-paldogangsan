...
async function scrapeGoseongMall() {
  console.log('🚀 Starting Goseong Mall comprehensive scraper...');
  ...
  try {
    ...
    $homepage('a[href*="/goods/view"]').each((i, elem) => {
      ...
      if (href) {
        ...
        // Handle both gwgoseong-mall.com and www.gwgoseong-mall.com
        fullUrl = fullUrl.replace('www.gwgoseong-mall.com', 'gwgoseong-mall.com');
    ...
    const productsPath = path.join(outputDir, 'goseong-products.json');
    ...
    const summaryPath = path.join(outputDir, 'goseong-scrape-summary.json');
...
scrapeGoseongMall()
  .then((products) => {
    console.log(`\n🎉 Goseong Mall scraping completed! Found ${products.length} products.`);
  })
  .catch((error) => {
