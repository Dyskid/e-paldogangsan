...
function verifyRegistration() {
  try {
    ...
    // Filter 초록믿음(강진) products
    const greengjProducts = products.filter(p => p.mall && p.mall.mallName === '초록믿음(강진)');
    
    console.log(`Found ${greengjProducts.length} products from 초록믿음(강진)`);
    
    // Verification checks
    const issues: string[] = [];
    
    // Check for invalid prices
    const invalidPrices = greengjProducts.filter(p => !p.price || p.price <= 0);
    ...
    const missingImages = greengjProducts.filter(p => !p.image);
    if (missingImages.length > 0) {
      issues.push(`${missingImages.length} products with missing images`);
    }
    
    // Check for missing URLs
    const missingUrls = greengjProducts.filter(p => !p.url);
    if (missingUrls.length > 0) {
      issues.push(`${missingUrls.length} products with missing URLs`);
    }
    
    // Check for duplicate URLs
    const urlCounts = greengjProducts.reduce((acc, p) => {
    ...
    const suspiciousProducts = greengjProducts.filter(p => {
    ...
    const categoryDistribution = greengjProducts.reduce((acc, p) => {
    ...
    const prices = greengjProducts.map(p => p.price).filter(p => p > 0);
    ...
    greengjProducts.slice(0, 10).forEach(p => {
    ...
    const summary = {
      ...
      mall: '초록믿음(강진)',
      totalProducts: greengjProducts.length,
      issues: issues,
    ...
    writeFileSync('./scripts/output/greengj-verification-report.json', JSON.stringify(summary, null, 2));
...
if (result) {
  console.log('\nVerification completed. Report saved to greengj-verification-report.json');
}