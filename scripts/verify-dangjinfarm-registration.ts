...
async function verifyDangjinfarmRegistration() {
  try {
    console.log('Starting 당진팜 registration verification...');
    ...
    // Filter 당진팜 products
    const dangjinfarmProducts = allProducts.filter(p => p.id.startsWith('dangjinfarm_'));
    
    console.log(`Found ${dangjinfarmProducts.length} 당진팜 products in database`);
    
    // Verification checks
    const verificationResults = {
      totalProducts: dangjinfarmProducts.length,
      validProducts: 0,
      invalidProducts: 0,
      issues: [] as string[],
    ...
    // Check each product
    dangjinfarmProducts.forEach((product, index) => {
      let isValid = true;
      ...
      if (!product.url || !product.url.startsWith('https://dangjinfarm.com')) {
    ...
    const potentialCategories = dangjinfarmProducts.filter(p => 
    ...
    const report = {
      ...
      sampleProducts: dangjinfarmProducts.slice(0, 3).map(p => ({
        id: p.id,
        title: p.title,
        price: p.price,
        category: p.category,
    ...
    const reportPath = path.join(__dirname, 'output', 'dangjinfarm-verification-report.json');
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
    
    // Print summary
    console.log('\n=== 당진팜 VERIFICATION REPORT ===');
    ...
    console.log('당진팜 verification completed!');
