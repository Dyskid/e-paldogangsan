...
async function scrapeUlmall() {
  console.log('Starting comprehensive scrape of 울릉도몰...');
  ...
  try {
    ...
    for (let i = 0; i < Math.min(uniqueUrls.length, 50); i++) {
      ...
      try {
        ...
        // Extract price - 울릉도몰 uses same CYSO structure as 영양온심마켓
        let price = '';
        ...
        const productId = productIdMatch ? productIdMatch[1] : `ulmall_${Date.now()}_${i}`;
    ...
    const outputFile = path.join(outputDir, 'ulmall-products.json');
    fs.writeFileSync(outputFile, JSON.stringify(products, null, 2));
    
    const summaryFile = path.join(outputDir, 'ulmall-scrape-summary.json');
...
if (require.main === module) {
  scrapeUlmall().catch(console.error);
}
