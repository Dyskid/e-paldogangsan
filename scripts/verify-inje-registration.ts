...
function verifyInjeRegistration(): void {
  try {
    console.log('🔍 Starting Inje Mall registration verification...');
    ...
    const injeProducts = allProducts.filter(product => 
      product.mall === '인제몰' || 
      product.id.startsWith('inje_') ||
      (product.url && product.url.includes('inje-mall.com'))
    );
    
    console.log(`🏪 Inje Mall products found: ${injeProducts.length}`);
    
    if (injeProducts.length === 0) {
      console.log('❌ No Inje Mall products found in database!');
      return;
    }
    
    const productsWithPrices = injeProducts.filter(p => p.price && p.price.trim() !== '');
    const productsWithImages = injeProducts.filter(p => p.image && p.image.trim() !== '');
    const productsWithTags = injeProducts.filter(p => p.tags && p.tags.length > 0);
    
    const uniqueCategories = [...new Set(injeProducts.map(p => p.category))];
    const allTags = injeProducts.flatMap(p => p.tags || []);
    ...
    const verificationResult: VerificationResult = {
      ...
      injeProducts: injeProducts.length,
      ...
      uniqueTags,
      priceDistribution,
      sampleProducts: injeProducts.slice(0, 5).map(p => ({
        id: p.id,
        name: p.name,
        price: p.price,
        tags: p.tags
    ...
    const verificationFile = path.join(outputDir, 'inje-verification-report.json');
    fs.writeFileSync(verificationFile, JSON.stringify(verificationResult, null, 2), 'utf8');
    
    console.log('\n✅ Inje Mall registration verification completed!');
    console.log(`\n📈 Verification Results:`);
    console.log(`   • Total products in database: ${verificationResult.totalProductsInDatabase}`);
    console.log(`   • Inje Mall products: ${verificationResult.injeProducts}`);
    ...
    if (verificationResult.injeProducts >= 35 && issues.length === 0) {
...
if (require.main === module) {
  verifyInjeRegistration();
}

export { verifyInjeRegistration };