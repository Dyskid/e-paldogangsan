...
async function scrapeHwasunfarmProductsQuick() {
  ...
  console.log(`Starting quick scrape of 화순팜 with ${categories.length} categories...`);
  ...
  for (const category of categories) {
    ...
    try {
      ...
      // Limit to first 15 products per category for speed
      const limitedLinks = productLinks.slice(0, 15);

      // Scrape each product
      for (const productLink of limitedLinks) {
        try {
          await delay(500); // Faster rate limiting
...
if (require.main === module) {
  scrapeHwasunfarmProductsQuick().catch(console.error);
}

export { scrapeHwasunfarmProductsQuick };