...
async function scrapeCgmallProducts() {
  console.log('🕸️ Starting comprehensive scraping of 칠곡몰...\n');
  ...
  try {
    ...
    const productsPath = path.join(outputDir, 'cgmall-products.json');
    ...
    const summaryPath = path.join(outputDir, 'cgmall-scrape-summary.json');
...
// Run scraper
scrapeCgmallProducts()
  .then((products) => {
    console.log(`\n✅ Successfully scraped ${products.length} products from 칠곡몰!`);
    process.exit(0);
  })
  .catch((error) => {
