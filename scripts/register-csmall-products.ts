...
async function registerCsmallProducts() {
  const timestamp = Date.now();
  console.log('Starting 칠곡몰 product registration...');
  
  try {
    // Load scraped products
    const scrapedPath = path.join(process.cwd(), 'scripts/output/csmall-products.json');
    ...
    const filteredExisting = existingProducts.filter(p => p.mallId !== 'csmall');
    console.log(`Removed ${existingProducts.length - filteredExisting.length} existing 칠곡몰 products`);
    ...
    // Save summary
    const summaryPath = path.join(process.cwd(), 'scripts/output/csmall-registration-summary.json');
...
registerCsmallProducts().catch(console.error);