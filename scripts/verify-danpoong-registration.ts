...
function main() {
  console.log('=== Verifying 단풍미인 Product Registration ===');
  ...
  // Filter 단풍미인 products
  const danpoongProducts = allProducts.filter(product => product.mall === '단풍미인');
  
  console.log(`Total products in database: ${allProducts.length}`);
  console.log(`단풍미인 products: ${danpoongProducts.length}`);

  // Data quality checks
  const dataQualityChecks = {
    productsWithPrices: danpoongProducts.filter(p => p.price && p.price !== '0').length,
    productsWithImages: danpoongProducts.filter(p => p.image && p.image.length > 0).length,
    productsWithDescriptions: danpoongProducts.filter(p => p.description && p.description.length > 0).length,
    productsWithCategories: danpoongProducts.filter(p => p.category && p.category.length > 0).length,
    duplicateUrls: 0,
    invalidProducts: 0
  };
  ...
  danpoongProducts.forEach(product => {
  ...
  danpoongProducts.forEach((product, index) => {
  ...
  const categorizedProducts = danpoongProducts.reduce((acc, product) => {
  ...
  if (dataQualityChecks.productsWithPrices / danpoongProducts.length < 1) {
    recommendations.push('Some products are missing prices - consider re-scraping or manual entry');
  }
  if (dataQualityChecks.productsWithImages / danpoongProducts.length < 0.8) {
  ...
  const report: VerificationReport = {
    ...
    mall: '단풍미인',
    totalDanpoongProducts: danpoongProducts.length,
  ...
  fs.writeFileSync(
    'scripts/output/danpoong-verification-report.json',
  ...
  console.log(`단풍미인 products verified: ${danpoongProducts.length}`);
  console.log('\nData Quality:');
  console.log(`  Products with prices: ${dataQualityChecks.productsWithPrices}/${danpoongProducts.length} (${((dataQualityChecks.productsWithPrices/danpoongProducts.length)*100).toFixed(1)}%)`);
  console.log(`  Products with images: ${dataQualityChecks.productsWithImages}/${danpoongProducts.length} (${((dataQualityChecks.productsWithImages/danpoongProducts.length)*100).toFixed(1)}%)`);
  console.log(`  Products with categories: ${dataQualityChecks.productsWithCategories}/${danpoongProducts.length} (${((dataQualityChecks.productsWithCategories/danpoongProducts.length)*100).toFixed(1)}%)`);
  ...
  console.log(`\n✓ Verification report saved to danpoong-verification-report.json`);
  
  // Final assessment
  const qualityScore = (
    (dataQualityChecks.productsWithPrices / danpoongProducts.length) * 0.4 +
    (dataQualityChecks.productsWithImages / danpoongProducts.length) * 0.2 +
    (dataQualityChecks.productsWithCategories / danpoongProducts.length) * 0.2 +
    ((danpoongProducts.length - dataQualityChecks.invalidProducts) / danpoongProducts.length) * 0.2
  ) * 100;
