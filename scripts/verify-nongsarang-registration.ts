...
async function verifyNongsarangRegistration() {
  try {
    console.log('Starting 농사랑 registration verification...');
    ...
    // Filter 농사랑 products
    const nongsarangProducts = allProducts.filter(p => p.id.startsWith('nongsarang_'));
    
    console.log(`Found ${nongsarangProducts.length} 농사랑 products in database`);
    
    // Verification checks
    const verificationResults = {
      totalProducts: nongsarangProducts.length,
      validProducts: 0,
      invalidProducts: 0,
      issues: [] as string[],
    ...
    // Check each product
    nongsarangProducts.forEach((product, index) => {
      let isValid = true;
      ...
      if (!product.url || !product.url.startsWith('https://nongsarang.co.kr') && !product.url.startsWith('https://www.nongsarang.co.kr')) {
    ...
    const potentialCategories = nongsarangProducts.filter(p => 
    ...
    const report = {
      ...
      sampleProducts: nongsarangProducts.slice(0, 3).map(p => ({
        id: p.id,
        title: p.title,
        price: p.price,
        category: p.category,
    ...
    const reportPath = path.join(__dirname, 'output', 'nongsarang-verification-report.json');
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
    
    // Print summary
    console.log('\n=== 농사랑 VERIFICATION REPORT ===');
    ...
    console.log('농사랑 verification completed!');
