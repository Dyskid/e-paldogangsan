...
async function scrapeDonghaeMall() {
  console.log('🚀 Starting Donghae Mall comprehensive scraper...');
  ...
  try {
    ...
    const productsPath = path.join(outputDir, 'donghae-products.json');
    ...
    const summaryPath = path.join(outputDir, 'donghae-scrape-summary.json');
...
scrapeDonghaeMall()
  .then((products) => {
    console.log(`\n🎉 Donghae Mall scraping completed! Found ${products.length} products.`);
  })
  .catch((error) => {
