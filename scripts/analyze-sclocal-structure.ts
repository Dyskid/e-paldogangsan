...
async function analyzeSCLocalStructure() {
  const baseUrl = 'https://sclocal.kr';
  
  try {
    console.log('📊 Analyzing SC Local mall structure...');
    ...
    fs.writeFileSync('./scripts/output/sclocal-homepage.html', homepageResponse.data);
    console.log('✅ Homepage saved to sclocal-homepage.html');
    ...
    $('nav a, .gnb a, .category a, .menu a, .nav a').each((i, el) => {
    ...
    for (const selector of productSelectors) {
      ...
      if (products.length > 0) {
        ...
        const titleSelectors = ['.title', '.name', '.prd-name', '.goods-name', '.product-name', 'h3', 'h4', '.subject', '.prd_name'];
    ...
    if (bodyHtml.includes('고도몰') || bodyHtml.includes('godomall')) {
      analysis.platform = 'Godomall5';
      console.log('🔍 Detected Godomall5 platform');
    }
    ...
    // Save analysis
    fs.writeFileSync('./scripts/output/sclocal-structure-analysis.json', JSON.stringify(analysis, null, 2));
    console.log('✅ Analysis saved to sclocal-structure-analysis.json');
    
    return analysis;
    
  } catch (error) {
    console.error('❌ Error analyzing SC Local structure:', error);
    throw error;
  }
}

// Run the analysis
analyzeSCLocalStructure()
  .then(() => {
    console.log('\n🎉 SC Local structure analysis completed!');
    process.exit(0);
  })
  .catch((error) => {
