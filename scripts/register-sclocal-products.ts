...
async function registerSCLocalProducts() {
  try {
    console.log('🚀 Starting SC Local product registration...');
    ...
    console.log('\n🎉 SC Local product registration completed successfully!');
    return summary;
...
registerSCLocalProducts()
  .then((summary) => {
    console.log(`\n✅ Registration completed! Added ${summary.validProducts} products from SC Local.`);
    process.exit(0);
  })
  .catch((error) => {
