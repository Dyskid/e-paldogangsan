...
async function testHongcheonProduct() {
  console.log('🧪 Testing Hongcheon Mall product page structure...');
  
  try {
    // Test with a different product URL
    const testUrl = 'https://hongcheon-mall.com/goods/view?no=27952';
    ...
    fs.writeFileSync(
      path.join(outputDir, 'hongcheon-product-sample.html'),
      response.data,
      'utf-8'
    );
    ...
    const testResultsPath = path.join(outputDir, 'hongcheon-product-test-results.json');
