...
async function scrapeCysoProducts() {
  ...
  const categories = [
    ...
    { name: '수산물', url: 'shop/list.php?ca_id=50' },
    { name: '가공식품', url: 'shop/list.php?ca_id=60' },
    { name: '김치/장류/참기름', url: 'shop/list.php?ca_id=80' },
    { name: '한과/떡/빵류', url: 'shop/list.php?ca_id=90' }
  ];

  console.log(`Starting scrape of 사이소(경북몰) with ${categories.length} categories...`);
  ...
  for (const category of categories) {
    ...
    try {
      ...
      // Limit to first 15 products per category for manageable scraping
      const limitedLinks = productLinks.slice(0, 15);
...
if (require.main === module) {
  scrapeCysoProducts().catch(console.error);
}

export { scrapeCysoProducts };