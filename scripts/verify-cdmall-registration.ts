...
async function verifyCdmallRegistration() {
  const timestamp = Date.now();
  console.log('Starting 청도몰 registration verification...');
  ...
  try {
    ...
    // Filter 청도몰 products
    const cdmallProducts = products.filter(p => p.mallId === 'cdmall');
    
    console.log(`\n=== VERIFICATION RESULTS ===`);
    console.log(`Total 청도몰 products in database: ${cdmallProducts.length}`);

    if (cdmallProducts.length === 0) {
      console.log('❌ No 청도몰 products found in database!');
      return;
    }

    // Analyze the registered products
    const categoryBreakdown = cdmallProducts.reduce((acc, product) => {
    ...
    const productsWithoutTitles = cdmallProducts.filter(p => !p.title || p.title.trim() === '');
    const productsWithoutImages = cdmallProducts.filter(p => !p.imageUrl && !p.image);
    const productsWithoutPrices = cdmallProducts.filter(p => !p.price || p.price === 0);
    const productsWithoutUrls = cdmallProducts.filter(p => !p.url);
    ...
    const urlCounts = cdmallProducts.reduce((acc, product) => {
    ...
    console.log(`\n=== SAMPLE PRODUCTS ===`);
    const sampleProducts = cdmallProducts.slice(0, 3);
    ...
    const validPrices = cdmallProducts
      .filter(p => p.price)
      .map(p => {
    ...
    // Check for specific 청도 products
    const specialProducts = {
      감말랭이: cdmallProducts.filter(p => p.title?.includes('감말랭이')).length,
      반건시: cdmallProducts.filter(p => p.title?.includes('반건시')).length,
      아이스홍시: cdmallProducts.filter(p => p.title?.includes('아이스홍시')).length,
      미나리: cdmallProducts.filter(p => p.title?.includes('미나리')).length
    };
    ...
    const verificationReport = {
      ...
      mallId: 'cdmall',
      mallName: '청도몰',
      totalProducts: cdmallProducts.length,
      dataQuality: {
    ...
    const outputPath = path.join(process.cwd(), 'scripts/output/cdmall-verification-report.json');
...
verifyCdmallRegistration().catch(console.error);