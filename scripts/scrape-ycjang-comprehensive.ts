...
async function scrapeYcjang() {
  console.log('Starting comprehensive scrape of 예천장터...');
  ...
  try {
    ...
    for (let i = 0; i < Math.min(uniqueUrls.length, 50); i++) {
      ...
      try {
        ...
        // Extract price - 예천장터 uses same CYSO structure
        let price = '';
        ...
        const productId = productIdMatch ? productIdMatch[1] : `ycjang_${Date.now()}_${i}`;
    ...
    const outputFile = path.join(outputDir, 'ycjang-products.json');
    fs.writeFileSync(outputFile, JSON.stringify(products, null, 2));
    
    const summaryFile = path.join(outputDir, 'ycjang-scrape-summary.json');
...
if (require.main === module) {
  scrapeYcjang().catch(console.error);
}
