...
async function registerBmallProducts() {
  console.log('Registering 봉화장터 products...');
  
  // Load scraped products
  const scrapedProductsPath = path.join(__dirname, 'output', 'bmall-products.json');
  ...
  // Remove existing bmall products to avoid duplicates
  const filteredExisting = existingProducts.filter(p => p.mallId !== 'bmall');
  console.log(`Removed ${existingProducts.length - filteredExisting.length} existing bmall products`);
  ...
  const summaryPath = path.join(__dirname, 'output', 'bmall-registration-summary.json');
...
if (require.main === module) {
  registerBmallProducts().catch(console.error);
}

export { registerBmallProducts };