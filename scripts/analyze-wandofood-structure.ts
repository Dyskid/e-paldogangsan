...
async function analyzeWandoFoodStructure() {
  const baseUrl = 'https://wandofood.go.kr';
  
  try {
    console.log('Analyzing 완도군이숍 structure...');
    ...
    writeFileSync('./scripts/output/wandofood-homepage.html', response.data);
    ...
    // Check for specific patterns in wandofood
    ...
    // Save analysis
    writeFileSync('./scripts/output/wandofood-structure-analysis.json', JSON.stringify(analysis, null, 2));
...
analyzeWandoFoodStructure().then(result => {
  if (result) {
    console.log('Analysis completed. Check wandofood-structure-analysis.json for details.');
  }
}).catch(console.error);