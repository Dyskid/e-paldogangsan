...
async function analyzeHoengseongStructure(): Promise<void> {
  try {
    console.log('🔍 Analyzing Hoengseong Mall structure...');
    
    const baseUrl = 'https://hoengseong-mall.com/';
    ...
    const analysisFile = path.join(outputDir, 'hoengseong-analysis.json');
    fs.writeFileSync(analysisFile, JSON.stringify(analysis, null, 2), 'utf8');
    
    // Save homepage HTML for manual inspection
    const homepageFile = path.join(outputDir, 'hoengseong-homepage.html');
    ...
    console.error('❌ Error analyzing Hoengseong Mall structure:', error);
...
export { analyzeHoengseongStructure };