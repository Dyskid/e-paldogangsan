...
async function analyzeJeongseonStructure(): Promise<void> {
  try {
    console.log('🔍 Analyzing Jeongseon Mall structure...');
    
    const baseUrl = 'https://jeongseon-mall.com/';
    ...
    const analysisFile = path.join(outputDir, 'jeongseon-analysis.json');
    fs.writeFileSync(analysisFile, JSON.stringify(analysis, null, 2), 'utf8');
    
    // Save homepage HTML for manual inspection
    const homepageFile = path.join(outputDir, 'jeongseon-homepage.html');
    ...
    console.error('❌ Error analyzing Jeongseon Mall structure:', error);
...
export { analyzeJeongseonStructure };