...
async function analyzeEsmallStructure() {
  const baseUrl = 'https://esmall.cyso.co.kr';
  
  console.log('Starting analysis of 의성장날 structure...');
  
  const outputDir = path.join(__dirname, '../../output/esmall');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'esmall-homepage.html'), response.data);
    ...
    // Add specific notes for 의성장날
    analysis.notes.push('의성군 특산물 전문몰');
    analysis.notes.push('CYSO 플랫폼 기반');
    analysis.notes.push('마늘, 농산물 특화');

    // Save analysis results
    const outputFile = path.join(outputDir, 'esmall-structure-analysis.json');
    ...
    console.error('Error analyzing 의성장날 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeEsmallStructure().catch(console.error);
}

export { analyzeEsmallStructure };