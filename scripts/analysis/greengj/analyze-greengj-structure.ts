...
async function analyzeGreengjStructure() {
  const baseUrl = 'https://greengj.com';
  
  try {
    console.log('Analyzing 초록믿음(강진) structure...');
    ...
    writeFileSync('./scripts/output/greengj-homepage.html', response.data);
    ...
    // Save analysis
    writeFileSync('./scripts/output/greengj-structure-analysis.json', JSON.stringify(analysis, null, 2));
...
analyzeGreengjStructure().then(result => {
  if (result) {
    console.log('Analysis completed. Check greengj-structure-analysis.json for details.');
  }
}).catch(console.error);