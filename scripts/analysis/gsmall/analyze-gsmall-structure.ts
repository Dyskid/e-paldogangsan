...
async function analyzeGsmallStructure() {
  const baseUrl = 'https://gsmall.cyso.co.kr';
  
  console.log('Starting analysis of 경산몰 structure...');
  
  const outputDir = path.join(__dirname, '../../output/gsmall');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'gsmall-homepage.html'), response.data);
    ...
    // Add specific notes for 경산몰
    analysis.notes.push('경산시 특산물 전문몰');
    analysis.notes.push('CYSO 플랫폼 기반');
    analysis.notes.push('대추, 복숭아 등 농산물 특화');

    // Save analysis results
    const outputFile = path.join(outputDir, 'gsmall-structure-analysis.json');
    ...
    console.error('Error analyzing 경산몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeGsmallStructure().catch(console.error);
}

export { analyzeGsmallStructure };