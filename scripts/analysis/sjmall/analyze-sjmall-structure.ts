...
async function analyzeSjmallStructure() {
  const baseUrl = 'https://sjmall.cyso.co.kr';
  
  console.log('Starting analysis of 상주몰 structure...');
  ...
  try {
    ...
    fs.writeFileSync('./scripts/output/sjmall-homepage.html', response.data);
    ...
    } else if (bodyText.includes('cyso') || baseUrl.includes('cyso')) {
      analysis.platform = 'CYSO';
      analysis.detectedPatterns.push('CYSO platform detected (similar to main cyso.co.kr)');
    }
    ...
    // Check if this is similar to the main cyso.co.kr structure
    if ($('a[href*="shop/item.php?it_id="]').length > 0) {
      analysis.detectedPatterns.push('Similar to main CYSO structure with shop/item.php?it_id= pattern');
    }

    // Save analysis results
    const outputFile = './scripts/output/sjmall-structure-analysis.json';
    ...
    console.log(`Homepage saved to: ./scripts/output/sjmall-homepage.html`);

    return analysis;

  } catch (error) {
    console.error('Error analyzing 상주몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeSjmallStructure().catch(console.error);
}

export { analyzeSjmallStructure };