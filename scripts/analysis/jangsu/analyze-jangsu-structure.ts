...
async function analyzeHomepage() {
  console.log('=== Analyzing 장수몰 (www.장수몰.com) Homepage ===');
  ...
  try {
    ...
    // Save homepage HTML
    fs.writeFileSync('scripts/output/jangsu-homepage.html', response.data, 'utf8');
      ...
      $(selector).find('a').each((index, element) => {
        ...
        if (link && text && text.length > 0) {
          ...
          if (link.includes('product') || link.includes('shop') || link.includes('goods') || 
            ...
            const fullUrl = link.startsWith('http') ? link : `https://www.장수몰.com${link}`;
    ...
    if (bodyHtml.includes('cafe24')) {
      ...
      platform = 'React/SPA';
    } else if (bodyHtml.includes('laravel')) {
      platform = 'Laravel';
    }
    ...
    // Look for images that might indicate products
    console.log('\n=== Image Analysis ===');
    const images = $('img[src*="product"], img[src*="goods"], img[src*="item"]');
    console.log(`Found ${images.length} product-related images`);
    
    // Create analysis summary
    const analysis = {
      timestamp: new Date().toISOString(),
      mall: '장수몰',
      url: 'https://www.장수몰.com/',
      platform: platform,
      title: $('title').text(),
    ...
    if (images.length > 0) {
      analysis.recommendations.push('Product images found - visual content available');
    }
    
    // Save analysis
    fs.writeFileSync(
      'scripts/output/jangsu-structure-analysis.json',
    ...
    console.log('Analysis saved to jangsu-structure-analysis.json');
