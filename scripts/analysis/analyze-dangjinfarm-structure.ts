...
async function analyzeDangjinfarmStructure() {
  try {
    console.log('Starting analysis of 당진팜 (dangjinfarm.com)...');
    ...
    fs.writeFileSync(
      path.join(outputDir, 'dangjinfarm-homepage.html'),
      homeResponse.data
    );
    ...
    for (const url of testUrls) {
      try {
        ...
        if (response.status === 200 && response.data.length > 500) {
          ...
          // Save sample page
          fs.writeFileSync(
            path.join(outputDir, `dangjinfarm-${url.split('/').pop() || 'page'}.html`),
            response.data
          );
        }
      } catch (error) {
    ...
    // Save analysis
    const analysisPath = path.join(outputDir, 'dangjinfarm-analysis.json');
    fs.writeFileSync(analysisPath, JSON.stringify(analysis, null, 2));
    
    console.log('\n=== 당진팜 STRUCTURE ANALYSIS ===');
    ...
    console.log('당진팜 structure analysis completed!');
    
  } catch (error) {
    console.error('Error analyzing 당진팜 structure:', error);
    process.exit(1);
  }
}

analyzeDangjinfarmStructure();