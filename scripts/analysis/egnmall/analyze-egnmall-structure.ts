...
async function analyzeEgnmallStructure() {
  const baseUrl = 'https://egnmall.kr';
  
  console.log('Starting analysis of e경남몰 structure...');
  
  const outputDir = path.join(__dirname, '../../output/egnmall');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'egnmall-homepage.html'), response.data);
    ...
    // Add specific notes for e경남몰
    analysis.notes.push('경상남도 통합 온라인몰');
    analysis.notes.push('경남 전 지역 특산물 판매');
    analysis.notes.push('농수산물 및 가공품 종합몰');

    // Save analysis results
    const outputFile = path.join(outputDir, 'egnmall-structure-analysis.json');
    ...
    console.error('Error analyzing e경남몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeEgnmallStructure().catch(console.error);
}

export { analyzeEgnmallStructure };