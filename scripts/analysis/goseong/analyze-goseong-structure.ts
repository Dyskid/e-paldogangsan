...
async function analyzeGoseongStructure() {
  console.log('🔍 Starting Goseong Mall structure analysis...');
  
  const baseUrl = 'https://gwgoseong-mall.com';
  ...
  try {
    ...
    const analysisPath = path.join(outputDir, 'goseong-analysis.json');
    fs.writeFileSync(analysisPath, JSON.stringify(analysis, null, 2), 'utf-8');

    // Save sample pages
    const homepagePath = path.join(outputDir, 'goseong-homepage.html');
    fs.writeFileSync(homepagePath, analysis.samplePages.homepage, 'utf-8');

    if (analysis.samplePages.categoryPages.length > 0) {
      const categoryPath = path.join(outputDir, 'goseong-category-sample.html');
      fs.writeFileSync(categoryPath, analysis.samplePages.categoryPages[0], 'utf-8');
    }

    if (analysis.samplePages.productPages.length > 0) {
      const productPath = path.join(outputDir, 'goseong-product-sample.html');
...
// Run analysis
analyzeGoseongStructure()
  .then((result) => {
    console.log('\n🎉 Goseong Mall structure analysis completed!');
    console.log('\n📋 Next Steps:');
