...
async function analyzeTaebaekStructure(): Promise<void> {
  try {
    console.log('🔍 Analyzing Taebaek Mall structure...');
    
    const baseUrl = 'https://taebaek-mall.com/';
    ...
    const analysisFile = path.join(outputDir, 'taebaek-analysis.json');
    fs.writeFileSync(analysisFile, JSON.stringify(analysis, null, 2), 'utf8');
    
    // Save homepage HTML for manual inspection
    const homepageFile = path.join(outputDir, 'taebaek-homepage.html');
    ...
    console.error('❌ Error analyzing Taebaek Mall structure:', error);
...
export { analyzeTaebaekStructure };