...
async function analyzeHamyangStructure() {
  const baseUrl = 'https://2900.co.kr';
  
  console.log('Starting analysis of 함양몰 structure...');
  
  const outputDir = path.join(__dirname, '../../output/hamyang');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'hamyang-homepage.html'), response.data);
    ...
    $('a[href]').each((index, element) => {
      ...
      if (href && text) {
        ...
        if (href.includes('category') || href.includes('list') || 
            ...
            text.includes('함양') || text.includes('지리산')) {
    ...
    // Add specific notes for 함양몰
    analysis.notes.push('함양군 특산물 전문몰');
    analysis.notes.push('곶감, 양파, 사과 등 특화');
    analysis.notes.push('지리산 권역 농산물 판매');

    // Save analysis results
    const outputFile = path.join(outputDir, 'hamyang-structure-analysis.json');
    ...
    console.error('Error analyzing 함양몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeHamyangStructure().catch(console.error);
}

export { analyzeHamyangStructure };