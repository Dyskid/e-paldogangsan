...
async function analyzeJnmallStructure() {
  try {
    console.log('Fetching jnmall.kr...');
    
    // Fetch homepage - handle redirect to www subdomain
    const response = await axios.get('https://www.jnmall.kr/', {
      headers: {
    ...
    await fs.writeFile('scripts/output/jnmall-homepage.html', html);
    ...
    navSelectors.forEach(selector => {
      $(selector).each((_, elem) => {
        ...
        if (href && text && !href.includes('javascript:') && !href.startsWith('#')) {
          const fullUrl = href.startsWith('http') ? href : `https://www.jnmall.kr${href.startsWith('/') ? '' : '/'}${href}`;
    ...
    $('a[href*="/goods/"], a[href*="/product/"], a[href*="/item/"], a[href*="/shop/"]').each((_, elem) => {
      const $elem = $(elem);
      const href = $elem.attr('href');
      const text = $elem.text().trim();
    ...
    // Save analysis
    await fs.writeFile(
      'scripts/output/jnmall-structure-analysis.json',
    ...
    if (analysis.categoryLinks.length > 0) {
      ...
      try {
        ...
        await fs.writeFile('scripts/output/jnmall-category-sample.html', categoryHtml);
        ...
        productSelectors.forEach(selector => {
          $cat(selector).each((_, elem) => {
            ...
            const title = $elem.find('.name, .title, .product-name, .item_name').text().trim() ||
                         $elem.attr('title') ||
                         img.attr('alt') ||
                         '';
        ...
        if (analysis.productPatterns.length > 0) {
          ...
          try {
            ...
            await fs.writeFile('scripts/output/jnmall-product-sample.html', productHtml);
...
// Run the analysis
analyzeJnmallStructure().catch(console.error);