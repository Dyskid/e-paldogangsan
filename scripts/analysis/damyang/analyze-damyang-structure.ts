...
async function analyzeDamyangStructure() {
  const baseUrl = 'https://damyangmk.kr';
  
  try {
    console.log('Analyzing 담양장터 structure...');
    ...
    writeFileSync('./scripts/output/damyang-homepage.html', response.data);
    ...
    // Check for specific patterns in 담양장터
    ...
    // Save analysis
    writeFileSync('./scripts/output/damyang-structure-analysis.json', JSON.stringify(analysis, null, 2));
...
analyzeDamyangStructure().then(result => {
  if (result) {
    console.log('Analysis completed. Check damyang-structure-analysis.json for details.');
  }
}).catch(console.error);