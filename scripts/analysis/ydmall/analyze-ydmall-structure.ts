...
async function analyzeYdmallStructure() {
  const baseUrl = 'https://ydmall.cyso.co.kr';
  
  console.log('Starting analysis of 영덕장터 structure...');
  
  const outputDir = path.join(__dirname, '../../output/ydmall');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'ydmall-homepage.html'), response.data);
    ...
    // Add specific notes for 영덕장터
    analysis.notes.push('영덕군 특산물 전문몰');
    analysis.notes.push('CYSO 플랫폼 기반');
    analysis.notes.push('대게, 영덕 해산물 특화');

    // Save analysis results
    const outputFile = path.join(outputDir, 'ydmall-structure-analysis.json');
    ...
    console.error('Error analyzing 영덕장터 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeYdmallStructure().catch(console.error);
}

export { analyzeYdmallStructure };