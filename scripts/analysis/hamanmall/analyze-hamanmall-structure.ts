...
async function analyzeHamanmallStructure() {
  const baseUrl = 'https://hamanmall.com';
  
  console.log('Starting analysis of 함안몰 structure...');
  
  const outputDir = path.join(__dirname, '../../output/hamanmall');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'hamanmall-homepage.html'), response.data);
    ...
    $('a[href]').each((index, element) => {
      ...
      if (href && text) {
        ...
        if (href.includes('category') || href.includes('list') || 
            ...
            text.includes('함안') || text.includes('수박')) {
    ...
    // Add specific notes for 함안몰
    analysis.notes.push('함안군 특산물 전문몰');
    analysis.notes.push('함안수박, 농산물 특화');
    analysis.notes.push('지역 우수 농산물 판매');

    // Save analysis results
    const outputFile = path.join(outputDir, 'hamanmall-structure-analysis.json');
    ...
    console.error('Error analyzing 함안몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeHamanmallStructure().catch(console.error);
}

export { analyzeHamanmallStructure };