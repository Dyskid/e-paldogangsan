...
async function analyzeGjmallStructure() {
  const baseUrl = 'https://gjmall.cyso.co.kr';
  
  console.log('Starting analysis of 경주몰 structure...');
  
  const outputDir = path.join(__dirname, '../../output/gjmall');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'gjmall-homepage.html'), response.data);
    ...
    // Add specific notes for 경주몰
    analysis.notes.push('경주시 특산물 전문몰');
    analysis.notes.push('CYSO 플랫폼 기반');
    analysis.notes.push('한우, 문화재 상품 등 특화');

    // Save analysis results
    const outputFile = path.join(outputDir, 'gjmall-structure-analysis.json');
    ...
    console.error('Error analyzing 경주몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeGjmallStructure().catch(console.error);
}

export { analyzeGjmallStructure };