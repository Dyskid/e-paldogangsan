...
async function analyzeUlmallStructure() {
  const baseUrl = 'https://ulmall.cyso.co.kr';
  console.log('Analyzing 울릉도몰 structure...');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'ulmall-homepage.html'), response.data);
    ...
    const analysis = {
      mallName: '울릉도몰',
      baseUrl,
      platform: 'CYSO (Same as 사이소/상주몰/청도몰/영주장날/청송몰/영양온심마켓)',
      ...
      notes: [
        ...
        'Specializes in Ulleungdo island products (seafood, wild vegetables, etc.)'
      ]
    };
    
    console.log('\n=== 울릉도몰 Structure Analysis ===');
    ...
    const analysisPath = path.join(outputDir, 'ulmall-analysis.json');
    ...
    console.error('Error analyzing 울릉도몰:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeUlmallStructure().catch(console.error);
}

export { analyzeUlmallStructure };