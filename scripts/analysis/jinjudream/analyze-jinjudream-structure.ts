...
async function analyzeJinjudreamStructure() {
  const baseUrl = 'https://jinjudream.com';
  
  console.log('Starting analysis of 진주드림 structure...');
  
  const outputDir = path.join(__dirname, '../../output/jinjudream');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'jinjudream-homepage.html'), response.data);
    ...
    // Add specific notes for 진주드림
    analysis.notes.push('진주시 특산물 전문몰');
    analysis.notes.push('진주 실크, 농산물 특화');
    analysis.notes.push('지역 우수 상품 판매');

    // Save analysis results
    const outputFile = path.join(outputDir, 'jinjudream-structure-analysis.json');
    ...
    console.error('Error analyzing 진주드림 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeJinjudreamStructure().catch(console.error);
}

export { analyzeJinjudreamStructure };