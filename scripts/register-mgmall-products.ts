...
async function registerMgmallProducts() {
  console.log('📝 Starting registration of 문경몰 products...\n');

  // Read the scraped products
  const scrapedProductsPath = path.join(__dirname, 'output/mgmall-products.json');
  ...
  console.log('==============================');
  const mgmallProducts = newProducts.filter(p => p.mallId === 'mgmall');
...
// Run registration
registerMgmallProducts()
  .then((summary) => {
    console.log(`\n✅ Successfully registered ${summary.productsAdded} products from 문경몰!`);
