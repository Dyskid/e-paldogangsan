    ...
    try {
        ...
        kkimchiProducts.forEach(product => {
            ...
            if (product.name && product.name.trim())
                report.dataQuality.withTitles++;
            if (product.price > 0)
                report.dataQuality.withPrices++;
            if (product.image && !product.image.includes('no_image'))
                report.dataQuality.withImages++;
            if (product.url)
                report.dataQuality.withUrls++;
            if (product.category)
                report.dataQuality.withCategories++;
            if (product.price > 100 && product.price < 100000)
                report.dataQuality.withValidPrices++;
            if (product.originalPrice && product.originalPrice > product.price)
                report.dataQuality.withDiscounts++;
            if (product.description && product.description.length > 10)
                report.dataQuality.withDescriptions++;
            ...
            if (product.price > 0) {
                ...
                if (product.price < 10000)
                    report.priceAnalysis.priceRanges.under10k++;
                else if (product.price < 20000)
                    report.priceAnalysis.priceRanges['10k-20k']++;
                else if (product.price < 30000)
                    report.priceAnalysis.priceRanges['20k-30k']++;
                else
                    report.priceAnalysis.priceRanges.over30k++;
            }
        ...
        // Save report
        (0, fs_1.writeFileSync)('./scripts/output/kkimchi-food-verification-final.json', JSON.stringify(report, null, 2));
        // Print summary
        ...
        (0, fs_1.writeFileSync)('./scripts/output/kkimchi-food-verification-error.json', JSON.stringify(errorInfo, null, 2));
        throw error;
    }
}
// Run verification
