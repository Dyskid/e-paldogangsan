...
async function scrapeGreengjProducts() {
  const baseUrl = 'https://greengj.com';
  const products: Product[] = [];
  const scrapedUrls = new Set<string>();
  
  // Categories from the structure analysis
  const categories = [
    { name: '전체상품', url: '/category/전체상품/24/' },
    { name: '발효식품', url: '/category/발효식품/25/' },
    { name: '곡류', url: '/category/곡류/41/' },
    { name: '건강식품', url: '/category/건강식품/31/' },
    { name: '버섯류', url: '/category/버섯류/39/' },
    { name: '가공식품', url: '/category/가공식품/28/' },
    { name: '수산물', url: '/category/수산물/26/' },
    { name: '농/축산물', url: '/category/농축산물/27/' },
    { name: '강진쌀', url: '/category/강진쌀/29/' },
    { name: '쌀귀리', url: '/category/쌀귀리/30/' }
  ];
  
  try {
    console.log('Starting 초록믿음(강진) product scraping...');
    ...
    writeFileSync('./scripts/output/greengj-products.json', JSON.stringify(output, null, 2));
    console.log('Results saved to greengj-products.json');
...
// Main execution
scrapeGreengjProducts().then(products => {
  ...
  writeFileSync('./scripts/output/greengj-scrape-summary.json', JSON.stringify(summary, null, 2));
}).catch(console.error);