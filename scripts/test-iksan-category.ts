...
async function testCategoryPage() {
  console.log('=== Testing 익산몰 Category Page ===');
  
  // Test one of the main categories (쌀 category)
  const testUrl = 'https://iksanmall.com/goods/goods_list.php?cateCd=001001';
  ...
  try {
    ...
    fs.writeFileSync('scripts/output/iksan-category-sample.html', response.data, 'utf8');
    ...
    const productSelectors = [
      ...
      '.prodListArea', '.goodsItem', '.itemInfo',
    ...
    $('a').each((index, element) => {
      ...
      if (href && (href.includes('goods_view.php') || href.includes('detail') || href.includes('view'))) {
    ...
    fs.writeFileSync(
      'scripts/output/iksan-category-test-results.json',
      JSON.stringify(testSummary, null, 2),
      'utf8'
    );
    
    console.log('\n=== Test Complete ===');
    console.log('Results saved to iksan-category-test-results.json');
