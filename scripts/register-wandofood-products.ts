...
function isValidProduct(product: Product): boolean {
  ...
  const skipTerms = [
    ...
    '농산물', '수산물', '축산물', '가공식품', '카테고리', '완도전복',
    'icon', '더보기', '최근본상품', '##name##', '해조류', '간편식품'
  ];
  
  // Skip if price is suspiciously low (like 5원, 100원)
  if (price < 1000) return false;
  
  return !skipTerms.some(term => title.includes(term));
}

async function registerWandoFoodProducts() {
  try {
    console.log('Registering 완도군이숍 products...');
    
    // Read scraped products
    const scrapedProducts: Product[] = JSON.parse(
      readFileSync('./scripts/output/wandofood-products.json', 'utf-8')
    );
    ...
    writeFileSync('./scripts/output/wandofood-registration-summary.json', JSON.stringify(summary, null, 2));
...
registerWandoFoodProducts().then(summary => {
