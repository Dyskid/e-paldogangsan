...
function registerWemallFoodProducts() {
  ...
  try {
    ...
    const scrapedData = readFileSync('./scripts/output/wemall-food-products-final.json', 'utf-8');
    const scrapedResult: ScrapingResult = JSON.parse(scrapedData);
    ...
    writeFileSync('./scripts/output/wemall-food-registration-summary.json', JSON.stringify(summary, null, 2));
    ...
    writeFileSync('./scripts/output/wemall-food-registration-error.json', JSON.stringify(errorInfo, null, 2));
  }
}
