...
async function scrapeGangneungMall() {
  console.log('🚀 Starting Gangneung Mall comprehensive scraper...');
  ...
  try {
    ...
    for (let i = 0; i < Math.min(productUrls.length, 60); i++) {
      ...
      try {
        ...
        console.log(`✅ ${successCount} Scraped: ${title} - ${priceText}`);
    ...
    const productsPath = path.join(outputDir, 'gangneung-products.json');
    ...
    const summaryPath = path.join(outputDir, 'gangneung-scrape-summary.json');
...
scrapeGangneungMall()
  .then((products) => {
    console.log(`\n🎉 Gangneung Mall scraping completed! Found ${products.length} products.`);
  })
  .catch((error) => {
