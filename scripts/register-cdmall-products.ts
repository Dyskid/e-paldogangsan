...
async function registerCdmallProducts() {
  const timestamp = Date.now();
  console.log('Starting 청도몰 product registration...');
  
  try {
    // Load scraped products
    const scrapedPath = path.join(process.cwd(), 'scripts/output/cdmall-products.json');
    ...
    const filteredExisting = existingProducts.filter(p => p.mallId !== 'cdmall');
    console.log(`Removed ${existingProducts.length - filteredExisting.length} existing 청도몰 products`);
    ...
    // Save summary
    const summaryPath = path.join(process.cwd(), 'scripts/output/cdmall-registration-summary.json');
...
registerCdmallProducts().catch(console.error);