...
function verifyRegistration() {
  try {
    ...
    // Filter 함평천지몰 products
    const hampyeongProducts = products.filter(p => p.mall && p.mall.mallName === '함평천지몰');
    
    console.log(`Found ${hampyeongProducts.length} products from 함평천지몰`);
    
    // Verification checks
    const issues: string[] = [];
    
    // Check for invalid prices
    const invalidPrices = hampyeongProducts.filter(p => !p.price || p.price <= 0);
    ...
    const missingImages = hampyeongProducts.filter(p => !p.image);
    if (missingImages.length > 0) {
      issues.push(`${missingImages.length} products with missing images`);
    }
    
    // Check for missing URLs
    const missingUrls = hampyeongProducts.filter(p => !p.url);
    if (missingUrls.length > 0) {
      issues.push(`${missingUrls.length} products with missing URLs`);
    }
    
    // Check for duplicate URLs
    const urlCounts = hampyeongProducts.reduce((acc, p) => {
    ...
    const suspiciousProducts = hampyeongProducts.filter(p => {
    ...
    const categoryDistribution = hampyeongProducts.reduce((acc, p) => {
    ...
    const prices = hampyeongProducts.map(p => p.price).filter(p => p > 0);
    ...
    hampyeongProducts.slice(0, 10).forEach(p => {
    ...
    const summary = {
      ...
      mall: '함평천지몰',
      totalProducts: hampyeongProducts.length,
      issues: issues,
    ...
    writeFileSync('./scripts/output/hampyeong-verification-report.json', JSON.stringify(summary, null, 2));
...
if (result) {
  console.log('\nVerification completed. Report saved to hampyeong-verification-report.json');
}