...
async function scrapeOnsiMall() {
  console.log('Starting comprehensive scrape of 영양온심마켓...');
  ...
  try {
    ...
    for (let i = 0; i < Math.min(uniqueUrls.length, 50); i++) {
      ...
      try {
        ...
        // Extract price - 영양온심마켓 uses table structure
        let price = '';
        
        // Check JavaScript variables first
        ...
        const productId = productIdMatch ? productIdMatch[1] : `onsim_${Date.now()}_${i}`;
    ...
    const outputFile = path.join(outputDir, 'onsim-products.json');
    fs.writeFileSync(outputFile, JSON.stringify(products, null, 2));
    
    const summaryFile = path.join(outputDir, 'onsim-scrape-summary.json');
...
if (require.main === module) {
  scrapeOnsiMall().catch(console.error);
}

export { scrapeOnsiMall };