...
async function verifyCsmallRegistration() {
  const timestamp = Date.now();
  console.log('Starting 칠곡몰 registration verification...');
  ...
  try {
    ...
    // Filter 칠곡몰 products
    const csmallProducts = products.filter(p => p.mallId === 'csmall');
    
    console.log(`\n=== VERIFICATION RESULTS ===`);
    console.log(`Total 칠곡몰 products in database: ${csmallProducts.length}`);

    if (csmallProducts.length === 0) {
      console.log('❌ No 칠곡몰 products found in database!');
      return;
    }

    // Analyze the registered products
    const categoryBreakdown = csmallProducts.reduce((acc, product) => {
    ...
    const productsWithoutTitles = csmallProducts.filter(p => !p.title || p.title.trim() === '');
    const productsWithoutImages = csmallProducts.filter(p => !p.imageUrl && !p.image);
    const productsWithoutPrices = csmallProducts.filter(p => !p.price || p.price === 0);
    const productsWithoutUrls = csmallProducts.filter(p => !p.url);
    ...
    const urlCounts = csmallProducts.reduce((acc, product) => {
    ...
    console.log(`\n=== SAMPLE PRODUCTS ===`);
    const sampleProducts = csmallProducts.slice(0, 3);
    ...
    const validPrices = csmallProducts
      .filter(p => p.price)
      .map(p => {
    ...
    // Check for specific 청송 products
    const specialProducts = {
      사과: csmallProducts.filter(p => p.title?.includes('사과')).length,
      청송: csmallProducts.filter(p => p.title?.includes('청송')).length,
      사과즙: csmallProducts.filter(p => p.title?.includes('사과즙')).length,
      유과: csmallProducts.filter(p => p.title?.includes('유과')).length,
      한과: csmallProducts.filter(p => p.title?.includes('한과')).length,
      꿀: csmallProducts.filter(p => p.title?.includes('꿀')).length
    };
    ...
    const verificationReport = {
      ...
      mallId: 'csmall',
      mallName: '칠곡몰',
      totalProducts: csmallProducts.length,
      dataQuality: {
    ...
    const outputPath = path.join(process.cwd(), 'scripts/output/csmall-verification-report.json');
...
verifyCsmallRegistration().catch(console.error);