...
async function registerGokseongmallProducts() {
  const timestamp = Date.now();
  console.log('Starting 곡성몰 product registration...');

  // Read scraped products
  const productsFile = './scripts/output/gokseongmall-products.json';
  ...
  // Remove any existing 곡성몰 products to avoid duplicates
  const nonGokseongmallProducts = existingProducts.filter(p => p.mallId !== 'gokseongmall');
  console.log(`Removed ${existingProducts.length - nonGokseongmallProducts.length} existing 곡성몰 products`);

  // Add new products
  const updatedProducts = [...nonGokseongmallProducts, ...validProducts];
  ...
  const summaryPath = './scripts/output/gokseongmall-registration-summary.json';
...
if (require.main === module) {
  registerGokseongmallProducts().catch(console.error);
}

export { registerGokseongmallProducts };