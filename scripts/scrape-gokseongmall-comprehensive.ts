...
async function scrapeGokseongmallProducts() {
  ...
  const categories = [
    ...
    { name: '관광', url: 'kwa-ABS_goods_l-1012' },
    { name: '여행', url: 'kwa-ABS_goods_l-1008' },
    { name: '베스트상품', url: 'kwa-ABS_goods_l-1004' },
    { name: '멜론왔썸', url: 'kwa-ABS_goods_l-1010' }
  ];

  console.log(`Starting scrape of 곡성몰 with ${categories.length} categories...`);
  ...
  for (const category of categories) {
    ...
    try {
      ...
      // Limit to first 20 products per category for manageable scraping
...
if (require.main === module) {
  scrapeGokseongmallProducts().catch(console.error);
}

export { scrapeGokseongmallProducts };