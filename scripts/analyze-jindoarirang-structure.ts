...
async function analyzeJindoArirangMallStructure() {
  const baseUrl = 'https://jindoarirangmall.com';
  
  try {
    console.log('Analyzing 진도아리랑몰 structure...');
    ...
    writeFileSync('./scripts/output/jindoarirang-homepage.html', response.data);
    ...
    $('a[href*="category"], a[href*="goods"], a[href*="product"], a[href*="list"]').each((i, el) => {
    ...
    $('a[href*="view"], a[href*="detail"], a[href*="goods"], a[href*="product"]').each((i, el) => {
      const href = $(el).attr('href');
    ...
    // Save analysis
    writeFileSync('./scripts/output/jindoarirang-structure-analysis.json', JSON.stringify(analysis, null, 2));
...
analyzeJindoArirangMallStructure().then(result => {
  if (result) {
    console.log('Analysis completed. Check jindoarirang-structure-analysis.json for details.');
  }
}).catch(console.error);