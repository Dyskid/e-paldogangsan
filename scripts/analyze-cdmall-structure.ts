...
async function analyzeCdmallStructure() {
  const baseUrl = 'https://cdmall.cyso.co.kr';
  
  console.log('Analyzing 청도몰 (청리브) structure...');
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(outputDir, 'cdmall-homepage.html'),
      response.data
    );
    ...
    if (searchForm.length > 0) {
      ...
      analysis.searchUrl = searchForm.attr('action');
    }
    ...
    if (productLinks.length > 0) {
      try {
        ...
        fs.writeFileSync(
          path.join(outputDir, 'cdmall-product-sample.html'),
          productResponse.data
        );
        
        // Analyze product page structure
        analysis.productPageStructure = {
          hasTitle: $product('h1, .it_name, [class*="title"]').length > 0,
          hasPrice: $product(':contains("원")').length > 0,
          hasImage: $product('img[src*="item"], img[src*="product"]').length > 0,
          titleSelectors: [],
          priceSelectors: [],
          imageSelectors: []
        };
        ...
        // Find title
        ['h1', '.it_name', '[class*="title"]', 'title'].forEach(selector => {
    ...
    // Save analysis
    fs.writeFileSync(
      path.join(outputDir, 'cdmall-analysis.json'),
      JSON.stringify(analysis, null, 2)
    );
    
    console.log('\n=== 청도몰 Structure Analysis ===');
    ...
    console.log('\nAnalysis saved to:', path.join(outputDir, 'cdmall-analysis.json'));
    
  } catch (error) {
    console.error('Error analyzing 청도몰:', error);
  }
}

// Run the analysis
analyzeCdmallStructure().catch(console.error);