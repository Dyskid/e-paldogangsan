...
async function fixChamdsImagesReal(): Promise<void> {
  ...
  try {
    ...
    for (const product of chamdsProducts) {
      ...
      const scrapedProduct = scrapedProducts.find((sp: any) => sp.id === product.id);
    ...
    writeFileSync('./scripts/output/chamds-image-real-fix-summary.json', JSON.stringify(summary, null, 2));
