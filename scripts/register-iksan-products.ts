...
function main() {
  console.log('=== Registering 익산몰 Products ===');

  // Read scraped products
  const scrapedProducts: Product[] = JSON.parse(
    fs.readFileSync('scripts/output/iksan-products.json', 'utf8')
  );
  ...
  fs.writeFileSync(
    'scripts/output/iksan-registration-summary.json',
  ...
  console.log(`\n✓ Registration summary saved to iksan-registration-summary.json`);
}
