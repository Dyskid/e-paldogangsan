...
async function registerOnsimProducts() {
  console.log('Registering 영양온심마켓 products...');
  
  // Load scraped products
  const scrapedProductsPath = path.join(__dirname, 'output', 'onsim-products.json');
  ...
  // Remove existing onsim products to avoid duplicates
  const filteredExisting = existingProducts.filter(p => p.mallId !== 'onsim');
  console.log(`Removed ${existingProducts.length - filteredExisting.length} existing onsim products`);
  ...
  const summaryPath = path.join(__dirname, 'output', 'onsim-registration-summary.json');
...
if (require.main === module) {
  registerOnsimProducts().catch(console.error);
}

export { registerOnsimProducts };