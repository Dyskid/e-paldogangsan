...
async function analyzeGangneungStructure() {
  console.log('🔍 Starting Gangneung Mall structure analysis...');
  
  const baseUrl = 'https://gangneung-mall.com';
  ...
  try {
    ...
    const analysisPath = path.join(outputDir, 'gangneung-analysis.json');
    fs.writeFileSync(analysisPath, JSON.stringify(analysis, null, 2), 'utf-8');

    // Save sample pages
    const homepagePath = path.join(outputDir, 'gangneung-homepage.html');
    fs.writeFileSync(homepagePath, analysis.samplePages.homepage, 'utf-8');

    if (analysis.samplePages.categoryPages.length > 0) {
      const categoryPath = path.join(outputDir, 'gangneung-category-sample.html');
      fs.writeFileSync(categoryPath, analysis.samplePages.categoryPages[0], 'utf-8');
    }

    if (analysis.samplePages.productPages.length > 0) {
      const productPath = path.join(outputDir, 'gangneung-product-sample.html');
...
// Run analysis
analyzeGangneungStructure()
  .then((result) => {
    console.log('\n🎉 Gangneung Mall structure analysis completed!');
    console.log('\n📋 Next Steps:');
