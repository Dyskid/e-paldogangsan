...
function verifyHoengseongRegistration(): void {
  try {
    console.log('🔍 Starting Hoengseong Mall registration verification...');
    ...
    const hoengseongProducts = allProducts.filter(product => 
      product.mall === '횡성몰' || 
      product.id.startsWith('hoengseong_') ||
      (product.url && product.url.includes('hoengseong-mall.com'))
    );
    
    console.log(`🏪 Hoengseong Mall products found: ${hoengseongProducts.length}`);
    
    if (hoengseongProducts.length === 0) {
      console.log('❌ No Hoengseong Mall products found in database!');
      return;
    }
    
    const productsWithPrices = hoengseongProducts.filter(p => p.price && p.price.trim() !== '');
    const productsWithImages = hoengseongProducts.filter(p => p.image && p.image.trim() !== '');
    const productsWithTags = hoengseongProducts.filter(p => p.tags && p.tags.length > 0);
    
    const uniqueCategories = [...new Set(hoengseongProducts.map(p => p.category))];
    const allTags = hoengseongProducts.flatMap(p => p.tags || []);
    ...
    hoengseongProducts.forEach(product => {
    ...
    const verificationResult: VerificationResult = {
      ...
      hoengseongProducts: hoengseongProducts.length,
      ...
      uniqueTags,
      priceDistribution,
      sampleProducts: hoengseongProducts.slice(0, 5).map(p => ({
        id: p.id,
        name: p.name,
        price: p.price,
        tags: p.tags
    ...
    const verificationFile = path.join(outputDir, 'hoengseong-verification-report.json');
    fs.writeFileSync(verificationFile, JSON.stringify(verificationResult, null, 2), 'utf8');
    
    console.log('\n✅ Hoengseong Mall registration verification completed!');
    console.log(`\n📈 Verification Results:`);
    console.log(`   • Total products in database: ${verificationResult.totalProductsInDatabase}`);
    console.log(`   • Hoengseong Mall products: ${verificationResult.hoengseongProducts}`);
    ...
    if (verificationResult.hoengseongProducts >= 40 && issues.length === 0) {
...
if (require.main === module) {
  verifyHoengseongRegistration();
}

export { verifyHoengseongRegistration };