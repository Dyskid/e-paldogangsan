...
async function analyzeCgmallStructure() {
  console.log('🔍 Starting analysis of 칠곡몰 (https://cgmall.cyso.co.kr)...\n');

  const baseUrl = 'https://cgmall.cyso.co.kr';
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'cgmall-homepage.html'), response.data);
    ...
    const analysis = {
      mallName: '칠곡몰',
      baseUrl,
      platform: isCysoPlatform ? 'CYSO (Same as 사이소/상주몰/청도몰/영주장날/청송몰/영양온심마켓/울릉도몰/봉화장터/예천장터/문경몰)' : 'Unknown/Custom',
    ...
    const analysisPath = path.join(outputDir, 'cgmall-analysis.json');
    ...
    console.error('❌ Error analyzing 칠곡몰:', error);
    throw error;
  }
}

// Run analysis
analyzeCgmallStructure()
  .then(() => {
    console.log('\n✅ 칠곡몰 structure analysis completed successfully!');
    process.exit(0);
  })
  .catch((error) => {
