...
/**
 * Regional mall scraper (S006)
 * Generic scraper for regional government mall sites
 */
async function scrapeRegionalMall(mall) {
  ...
  try {
    ...
    // Regional mall specific selectors
    const selectors = [
      '.product-list li',
      '.goods-list li',
      '.item-list li',
    ...
    for (const selector of selectors) {
      ...
      if (elements.length > 0) {
        elements.each((i, elem) => {
          const $elem = $(elem);
          const name = $elem.find('.name, .product-name, .goods-name, .title, .item-name').first().text().trim();
          const priceText = $elem.find('.price, .product-price, .cost, .amount').first().text();
