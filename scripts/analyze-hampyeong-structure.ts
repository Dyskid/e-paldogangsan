...
async function analyzeHampyeongStructure() {
  const baseUrl = 'https://hampyeongm.com'; // 함평천지몰 official URL
  
  try {
    console.log('Analyzing 함평천지몰 structure...');
    ...
    writeFileSync('./scripts/output/hampyeong-homepage.html', response.data);
    ...
    // Check for specific patterns in 함평천지몰
    ...
    // Save analysis
    writeFileSync('./scripts/output/hampyeong-structure-analysis.json', JSON.stringify(analysis, null, 2));
...
analyzeHampyeongStructure().then(result => {
  if (result) {
    console.log('Analysis completed. Check hampyeong-structure-analysis.json for details.');
  }
}).catch(console.error);