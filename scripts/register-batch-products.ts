...
function extractTags(product: ScrapedProduct): string[] {
  ...
  if (product.mallName.includes('당진')) tags.add('충청남도');
  if (product.mallName.includes('함양')) tags.add('경상남도');
  if (product.mallName.includes('함평')) tags.add('전라남도');
  if (product.mallName.includes('단풍미인')) tags.add('전라북도');
...
async function registerBatchProducts() {
  console.log('Starting batch product registration...');
  
  // Load scraped products
  const batchDir = path.join(__dirname, 'output', 'batch-scrape');
  const summaryPath = path.join(batchDir, 'batch-scrape-summary.json');
  
  if (!fs.existsSync(summaryPath)) {
    console.error('Batch scrape summary not found. Please run batch scraper first.');
    return;
  }
  ...
  fs.writeFileSync(
    path.join(batchDir, 'batch-registration-summary.json'),
...
if (require.main === module) {
  registerBatchProducts().catch(console.error);
}

export { registerBatchProducts };