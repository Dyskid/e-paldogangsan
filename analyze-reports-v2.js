...
allErrors.forEach(error => {
    if (error.includes('certificate'))
        errorTypes['SSL/Certificate issues'] = (errorTypes['SSL/Certificate issues'] || 0) + 1;
    else if (error.includes('timeout'))
        errorTypes['Timeout'] = (errorTypes['Timeout'] || 0) + 1;
    else if (error.includes('ECONNREFUSED'))
        errorTypes['Connection refused'] = (errorTypes['Connection refused'] || 0) + 1;
...
fs.writeFileSync('/mnt/c/Users/johndoe/Desktop/e-paldogangsan/scraper-schema-analysis.json', JSON.stringify(results, null, 2));
