...
function generateScraperCode(analysis: AnalysisData): string {
...
function generatePackageJson(analysis: AnalysisData): string {
  const requiresJS = analysis.dynamicLoading?.requiresJavaScript || analysis.requiresJavaScript || false;
...
function generateConfig(analysis: AnalysisData): string {
  return `{
...
function generateReadme(analysis: AnalysisData): string {
...
function generateReport(analysis: AnalysisData): string {
...
async function generateScraperForMall(mallId: number, mallData: any) {
  try {
    // Find analysis file
    ...
    // Read analysis data
    const analysisData: AnalysisData = JSON.parse(fs.readFileSync(analysisPath, 'utf-8'));
...
async function main() {
  ...
  const mallsMap = new Map(mallsData.map((m: any) => [m.id, m]));
