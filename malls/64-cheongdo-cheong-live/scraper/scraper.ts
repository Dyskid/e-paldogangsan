...
class Mall64Scraper {
  private baseUrl = 'https://cdmall.cyso.co.kr';
  private mallId = 64;
  private mallName = 'cheongdo-cheong-live';
  ...
  private extractProducts($: cheerio.CheerioAPI, categoryName: string): number {
    ...
    for (const selector of productSelectors) {
      $(selector).each((i, el) => {
        ...
        const name = $product.find('.goods-header h3, .goods-name, .title').text().trim();
        const price = $product.find('.goods-price strong, .price-box .price').text().trim();
...
// Run the scraper
const scraper = new Mall64Scraper();
