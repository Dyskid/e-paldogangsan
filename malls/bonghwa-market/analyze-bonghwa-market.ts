...
async function analyzeBmallStructure() {
  const baseUrl = 'https://bmall.cyso.co.kr';
  console.log('Analyzing 봉화장터 structure...');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'bmall-homepage.html'), response.data);
    ...
    const analysis = {
      mallName: '봉화장터',
      baseUrl,
      platform: 'CYSO (Same as 사이소/상주몰/청도몰/영주장날/청송몰/영양온심마켓/울릉도몰)',
      ...
      notes: [
        ...
        'Specializes in Bonghwa regional products (herbs, mountain vegetables, etc.)'
      ]
    };
    
    console.log('\n=== 봉화장터 Structure Analysis ===');
    ...
    const analysisPath = path.join(outputDir, 'bmall-analysis.json');
    ...
    console.error('Error analyzing 봉화장터:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeBmallStructure().catch(console.error);
}

export { analyzeBmallStructure };