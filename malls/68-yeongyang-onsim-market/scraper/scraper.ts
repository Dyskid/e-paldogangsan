...
class Mall68Scraper {
  private baseUrl = 'https://onsim.cyso.co.kr/';
  private mallId = 68;
  private mallName = 'yeongyang-onsim-market';
  ...
  private extractProducts($: cheerio.CheerioAPI, categoryName: string): number {
    ...
    for (const selector of productSelectors) {
      $(selector).each((i, el) => {
        ...
        const name = $product.find('.goods-name, .item_tit_detail h3').text().trim();
        const price = $product.find('.goods-price strong, .price-real').text().trim();
...
// Run the scraper
const scraper = new Mall68Scraper();
