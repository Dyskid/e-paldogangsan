...
class YangjuMallAnalyzer {
  ...
  async analyze() {
    console.log('🔍 Analyzing Yangju Market structure...');
    ...
    try {
      ...
      fs.writeFileSync(path.join(outputDir, 'yangju-homepage.html'), response.data);
      
      // Extract mall name
      this.analysis.mallName = $('title').text().trim() || '양주시 온라인 마켓';
      ...
      // Save analysis
      const analysisFile = path.join(outputDir, 'yangju-analysis.json');
        ...
        elements.each((_, element) => {
          ...
          if (href && text && !seenUrls.has(href)) {
            ...
            const fullUrl = href.startsWith('http') ? href : 
                          href.startsWith('/') ? `https://market.yangju.go.kr${href}` :
                          `https://market.yangju.go.kr/${href}`;
    ...
    if (categories.length === 0) {
      ...
      for (const selector of listingSelectors) {
        ...
        if (elements.length > 0) {
          ...
          // Try to extract category info from the page structure
          break;
        }
      }
    }
      ...
      if (links.length > 0) {
        ...
        if (firstLink) {
          ...
          if (firstLink.includes('product_id=')) {
            this.analysis.productUrlPattern = 'https://market.yangju.go.kr/...?product_id={productId}';
          } else if (firstLink.includes('goods_id=')) {
            this.analysis.productUrlPattern = 'https://market.yangju.go.kr/...?goods_id={goodsId}';
          } else if (firstLink.includes('/view/')) {
            this.analysis.productUrlPattern = 'https://market.yangju.go.kr/.../view/{productId}';
          }
          break;
        }
      }
  ...
  private defineScrapeStrategy() {
    this.analysis.scrapingStrategy = {
      method: 'Category-based traversal with pagination',
      steps: [
        '1. Start from homepage to get category list',
        '2. For each category, fetch the listing page',
        '3. Extract product URLs from listing pages',
      ...
      estimatedProductCount: 'Unknown - requires category exploration'
    };
  }
...
// Run the analyzer
async function main() {
  const analyzer = new YangjuMallAnalyzer();
