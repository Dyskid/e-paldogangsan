...
async function analyzeGrmallStructure() {
  const baseUrl = 'https://grmall.cyso.co.kr';
  
  console.log('Starting analysis of 고령몰 structure...');
  
  const outputDir = path.join(__dirname, '../../output/grmall');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'grmall-homepage.html'), response.data);
    ...
    // Add specific notes for 고령몰
    analysis.notes.push('고령 지역 특산물 전문몰');
    analysis.notes.push('CYSO 플랫폼 기반');
    analysis.notes.push('딸기, 참외 등 농산물 특화');

    // Save analysis results
    const outputFile = path.join(outputDir, 'grmall-structure-analysis.json');
    ...
    console.error('Error analyzing 고령몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeGrmallStructure().catch(console.error);
}

export { analyzeGrmallStructure };