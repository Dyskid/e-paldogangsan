...
async function analyzePohangmarketStructure() {
  const baseUrl = 'https://pohangmarket.cyso.co.kr';
  
  console.log('Starting analysis of 포항마켓 structure...');
  
  const outputDir = path.join(__dirname, '../../output/pohangmarket');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'pohangmarket-homepage.html'), response.data);
    ...
    // Add specific notes for 포항마켓
    analysis.notes.push('포항시 특산물 전문몰');
    analysis.notes.push('CYSO 플랫폼 기반');
    analysis.notes.push('과메기, 해산물 등 특화');

    // Save analysis results
    const outputFile = path.join(outputDir, 'pohangmarket-structure-analysis.json');
    ...
    console.error('Error analyzing 포항마켓 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzePohangmarketStructure().catch(console.error);
}

export { analyzePohangmarketStructure };