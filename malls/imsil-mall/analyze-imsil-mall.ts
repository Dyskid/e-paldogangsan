...
async function analyzeImsilStructure() {
  const baseUrl = 'https://www.imsilin.kr';
  
  console.log('Starting analysis of 임실몰 structure...');
  
  const outputDir = path.join(__dirname, '../../output/imsil');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'imsil-homepage.html'), response.data);
    ...
    $('a[href]').each((index, element) => {
      ...
      if (href && text) {
        ...
        if (href.includes('category') || href.includes('list') || 
            ...
            text.includes('치즈') || text.includes('유제품')) {
    ...
    // Add specific notes for 임실몰
    analysis.notes.push('임실군 특산물 전문몰');
    analysis.notes.push('임실치즈, 유제품 특화');
    analysis.notes.push('지역 농산물 판매');

    // Save analysis results
    const outputFile = path.join(outputDir, 'imsil-structure-analysis.json');
    ...
    console.error('Error analyzing 임실몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeImsilStructure().catch(console.error);
}

export { analyzeImsilStructure };