...
async function analyzeChuncheonStructure(): Promise<void> {
  try {
    console.log('🔍 Analyzing Chuncheon Mall structure...');
    
    const baseUrl = 'https://gwch-mall.com/';
    ...
    const analysisFile = path.join(outputDir, 'chuncheon-analysis.json');
    fs.writeFileSync(analysisFile, JSON.stringify(analysis, null, 2), 'utf8');
    
    // Save homepage HTML for manual inspection
    const homepageFile = path.join(outputDir, 'chuncheon-homepage.html');
    ...
    console.error('❌ Error analyzing Chuncheon Mall structure:', error);
...
export { analyzeChuncheonStructure };