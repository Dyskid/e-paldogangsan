...
async function analyzeUjmallStructure() {
  const baseUrl = 'https://ujmall.cyso.co.kr';
  
  console.log('Starting analysis of 울진몰 structure...');
  
  const outputDir = path.join(__dirname, '../../output/ujmall');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'ujmall-homepage.html'), response.data);
    ...
    // Add specific notes for 울진몰
    analysis.notes.push('울진군 특산물 전문몰');
    analysis.notes.push('CYSO 플랫폼 기반');
    analysis.notes.push('대게, 해산물, 농산물 특화');

    // Save analysis results
    const outputFile = path.join(outputDir, 'ujmall-structure-analysis.json');
    ...
    console.error('Error analyzing 울진몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeUjmallStructure().catch(console.error);
}

export { analyzeUjmallStructure };