...
async function analyzeSanencheongStructure() {
  const baseUrl = 'https://sanencheong.com';
  
  console.log('Starting analysis of 산엔청 structure...');
  
  const outputDir = path.join(__dirname, '../../output/sanencheong');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'sanencheong-homepage.html'), response.data);
    ...
    $('a[href]').each((index, element) => {
      ...
      if (href && text) {
        ...
        if (href.includes('category') || href.includes('list') || 
            ...
            text.includes('산청') || text.includes('약초')) {
    ...
    // Add specific notes for 산엔청
    analysis.notes.push('산청군 특산물 전문몰');
    analysis.notes.push('약초, 한방재료 특화');
    analysis.notes.push('지리산 청정 농산물 판매');

    // Save analysis results
    const outputFile = path.join(outputDir, 'sanencheong-structure-analysis.json');
    ...
    console.error('Error analyzing 산엔청 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeSanencheongStructure().catch(console.error);
}

export { analyzeSanencheongStructure };