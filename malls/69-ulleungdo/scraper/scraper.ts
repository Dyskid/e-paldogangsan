...
class Mall69Scraper {
  private baseUrl = 'https://ulmall.cyso.co.kr';
  private mallId = 69;
  private mallName = 'ulleungdo';
  ...
  private extractProducts($: cheerio.CheerioAPI, categoryName: string): number {
    ...
    for (const selector of productSelectors) {
      $(selector).each((i, el) => {
        ...
        const name = $product.find('.goods-header .name, h3.goods-name').text().trim();
        const price = $product.find('.goods-price .price, .real-price strong').text().trim();
...
// Run the scraper
const scraper = new Mall69Scraper();
