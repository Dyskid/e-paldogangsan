...
async function extractProductsFromPage(html: string): Promise<Array<any>> {
...
async function analyzeMall(): Promise<MallAnalysis> {
  ...
  const newArrivalsHtml = await fetchPageContent(`${baseUrl}/goods/new_arrivals`);
  ...
  if (newArrivalsHtml) {
    ...
    $('.filter_brand_section label[data-value]').each((_, element) => {
      const $label = $(element);
      const brandId = $label.attr('data-value')?.replace('b', '') || '';
    ...
    const categoryInfo: Array<{code: string, name: string, count: number}> = [];
    $('.filter_category_section a[data-value]').each((_, element) => {
      const $link = $(element);
      const code = $link.attr('data-value')?.replace('c', '') || '';
    ...
    products = await extractProductsFromPage(newArrivalsHtml);
    ...
    if (products.length === 0) {
      ...
      const searchHtml = await fetchPageContent(searchUrl);
      if (searchHtml) {
        products = await extractProductsFromPage(searchHtml);
      }
    }
  }
