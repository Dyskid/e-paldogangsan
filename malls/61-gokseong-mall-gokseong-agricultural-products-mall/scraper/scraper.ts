...
class Mall61Scraper {
  private baseUrl = 'https://gokseongmall.com/';
  private mallId = 61;
  private mallName = 'gokseong-mall-gokseong-agricultural-products-mall';
  ...
  private extractProducts($: cheerio.CheerioAPI, categoryName: string): number {
    ...
    for (const selector of productSelectors) {
      $(selector).each((i, el) => {
        ...
        const name = $product.find('.product-name, .goods-name, [class*="name"]').text().trim();
        const price = $product.find('.price, .product-price, [class*="price"]').text().trim();
...
// Run the scraper
const scraper = new Mall61Scraper();
