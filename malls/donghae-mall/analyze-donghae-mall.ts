...
async function analyzeDonghaeStructure() {
  console.log('🔍 Starting Donghae Mall structure analysis...');
  ...
  try {
    ...
    const analysisPath = path.join(outputDir, 'donghae-analysis.json');
    fs.writeFileSync(analysisPath, JSON.stringify(analysis, null, 2), 'utf-8');

    // Save sample pages
    const homepagePath = path.join(outputDir, 'donghae-homepage.html');
    fs.writeFileSync(homepagePath, analysis.samplePages.homepage, 'utf-8');

    if (analysis.samplePages.categoryPages.length > 0) {
      const categoryPath = path.join(outputDir, 'donghae-category-sample.html');
      fs.writeFileSync(categoryPath, analysis.samplePages.categoryPages[0], 'utf-8');
    }

    if (analysis.samplePages.productPages.length > 0) {
      const productPath = path.join(outputDir, 'donghae-product-sample.html');
...
// Run analysis
analyzeDonghaeStructure()
  .then((result) => {
    console.log('\n🎉 Donghae Mall structure analysis completed!');
    console.log('\n📋 Next Steps:');
