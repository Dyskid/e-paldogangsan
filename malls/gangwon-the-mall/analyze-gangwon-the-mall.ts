...
class GwdMallAnalyzer {
  ...
  async analyze() {
    console.log('🔍 Analyzing GWDMall structure...');
    ...
    try {
      ...
      fs.writeFileSync(path.join(outputDir, 'gwdmall-homepage.html'), response.data);
      
      // Extract mall name
      this.analysis.mallName = $('title').text().trim() || 'GWD Mall';
      ...
      // Save analysis
      const analysisFile = path.join(outputDir, 'gwdmall-analysis.json');
        ...
        elements.each((_, element) => {
          ...
          if (href && text && !seenUrls.has(href)) {
            ...
            const fullUrl = href.startsWith('http') ? href : 
                          href.startsWith('/') ? `https://gwdmall.kr${href}` :
                          `https://gwdmall.kr/${href}`;
      ...
      if (links.length > 0) {
        ...
        if (firstLink) {
          ...
          if (firstLink.includes('product_id=')) {
            this.analysis.productUrlPattern = 'https://gwdmall.kr/...?product_id={productId}';
          } else if (firstLink.includes('goods_id=')) {
            this.analysis.productUrlPattern = 'https://gwdmall.kr/...?goods_id={goodsId}';
          } else if (firstLink.includes('/view/')) {
            this.analysis.productUrlPattern = 'https://gwdmall.kr/.../view/{productId}';
          }
          break;
        }
      }
  ...
  private async testCategoryPage() {
    ...
    if (this.analysis.categoryStructure.length > 0) {
      ...
      try {
        ...
        const outputDir = path.join(__dirname, 'output');
        fs.writeFileSync(path.join(outputDir, 'gwdmall-test-category.html'), response.data);
...
// Run the analyzer
async function main() {
  const analyzer = new GwdMallAnalyzer();
