...
class Mall63Scraper {
  private baseUrl = 'https://sjmall.cyso.co.kr/';
  private mallId = 63;
  private mallName = 'sangju-myeongsil-sangju-mall';
  ...
  private extractProducts($: cheerio.CheerioAPI, categoryName: string): number {
    ...
    for (const selector of productSelectors) {
      $(selector).each((i, el) => {
        ...
        const name = $product.find('.goods-header h3, .goods-name, .item_tit_detail').text().trim();
        const price = $product.find('.goods-price, .price strong, .detail-price').text().trim();
...
// Run the scraper
const scraper = new Mall63Scraper();
