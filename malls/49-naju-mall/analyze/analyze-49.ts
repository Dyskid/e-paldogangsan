...
async function analyzeMall(): Promise<AnalysisResult> {
  const mallId = 49;
  const mallName = 'naju-mall';
  const url = 'https://naju-mall.com';
  ...
  try {
    ...
    // Check for specific naju-mall patterns
    const naju-mallProducts = document.querySelectorAll('.xans-product-listmain li, .xans-product li');
    if (naju-mallProducts.length > 0) {
      result.platform = 'Cafe24'; // xans- prefix is typical for Cafe24
      naju-mallProducts.forEach((item: Element, index: number) => {
        if (index >= 5) return;
...
(async () => {
  ...
  const outputPath = path.join(__dirname, 'analysis-49.json');
  fs.writeFileSync(outputPath, JSON.stringify(result, null, 2));
  
  console.log('Analysis completed for naju-mall (ID: 49)');
