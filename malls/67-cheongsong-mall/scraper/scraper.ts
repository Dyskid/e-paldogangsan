...
class Mall67Scraper {
  private baseUrl = 'https://csmall.cyso.co.kr/';
  private mallId = 67;
  private mallName = 'cheongsong-mall';
  ...
  private extractProducts($: cheerio.CheerioAPI, categoryName: string): number {
    ...
    for (const selector of productSelectors) {
      $(selector).each((i, el) => {
        ...
        const name = $product.find('.goods-header h3, .goods-name').text().trim();
        const price = $product.find('.goods-price .price, .detail-price').text().trim();
...
// Run the scraper
const scraper = new Mall67Scraper();
