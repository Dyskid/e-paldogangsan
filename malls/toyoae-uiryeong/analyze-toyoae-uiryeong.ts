...
async function analyzeToyoaeStructure() {
  const baseUrl = 'https://toyoae.com';
  
  console.log('Starting analysis of 토요애 structure...');
  
  const outputDir = path.join(__dirname, '../../output/toyoae');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'toyoae-homepage.html'), response.data);
    ...
    // Add specific notes for 토요애
    analysis.notes.push('토산품 전문 온라인몰');
    analysis.notes.push('지역 특산물 판매');
    analysis.notes.push('친환경 농산물 중심');

    // Save analysis results
    const outputFile = path.join(outputDir, 'toyoae-structure-analysis.json');
    ...
    console.error('Error analyzing 토요애 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeToyoaeStructure().catch(console.error);
}

export { analyzeToyoaeStructure };