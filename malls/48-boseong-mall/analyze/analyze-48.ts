...
async function analyzeMall(): Promise<AnalysisResult> {
  const mallId = 48;
  const mallName = 'boseong-mall';
  const url = 'https://boseong-mall.com';
  ...
  try {
    ...
    // Check for specific boseong-mall patterns
    const boseong-mallProducts = document.querySelectorAll('.xans-product-listmain li, .xans-product li');
    if (boseong-mallProducts.length > 0) {
      result.platform = 'Cafe24'; // xans- prefix is typical for Cafe24
      boseong-mallProducts.forEach((item: Element, index: number) => {
        if (index >= 5) return;
...
(async () => {
  ...
  const outputPath = path.join(__dirname, 'analysis-48.json');
  fs.writeFileSync(outputPath, JSON.stringify(result, null, 2));
  
  console.log('Analysis completed for boseong-mall (ID: 48)');
