...
async function analyzeMall(): Promise<MallAnalysis> {
  const mallId = 42;
  const mallName = '임실몰';
  const mallUrl = 'https://www.imsilin.kr/home';
  ...
  try {
    ...
    $('.gnb-menu > li, .nav-menu > li, .category-list > li, #gnb > li').each((i, elem) => {
      const $elem = $(elem);
      ...
      if (name && url) {
        const category = {
          name,
          url: url.startsWith('http') ? url : `https://www.imsilin.kr${url}`,
        ...
        $elem.find('.sub-menu > li > a, .dropdown-menu > li > a, ul > li > a').each((j, subElem) => {
          ...
          if (subName && subUrl) {
            category.subcategories!.push({
              name: subName,
              url: subUrl.startsWith('http') ? subUrl : `https://www.imsilin.kr${subUrl}`
            });
          }
        });
    ...
    for (const selector of listSelectors) {
      if ($(selector).length > 0) {
        ...
        // Find item selector
        const itemSelectors = ['li', '.item', '.product', '.goods', '.prd-item'];
        for (const itemSel of itemSelectors) {
          if ($(`${selector} ${itemSel}`).length > 0) {
            ...
            // Name selectors
            const nameSelectors = ['.name', '.title', '.prd-name', '.item-name', '.product-name', 'h3', 'h4', '.description strong'];
            ...
            // Price selectors
            const priceSelectors = ['.price', '.cost', '.prd-price', '.item-price', '.product-price', '.xans-product-price'];
    ...
    if ($('.pagination, .paging, .page-nav, .xans-product-normalpaging').length > 0) {
      pagination = {
        type: 'page',
        selector: '.pagination, .paging, .page-nav, .xans-product-normalpaging',
    ...
    fs.writeFileSync(
      path.join(__dirname, 'analysis-42.json'),
