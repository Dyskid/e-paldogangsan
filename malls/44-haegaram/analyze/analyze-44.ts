...
async function analyzeMall(): Promise<AnalysisResult> {
  const mallId = 44;
  const mallName = 'haegaram';
  const url = 'https://haegaram.com';
  ...
  try {
    ...
    // Check for specific haegaram patterns
    const haegaramProducts = document.querySelectorAll('.xans-product-listmain li, .xans-product li');
    if (haegaramProducts.length > 0) {
      result.platform = 'Cafe24'; // xans- prefix is typical for Cafe24
      haegaramProducts.forEach((item: Element, index: number) => {
        if (index >= 5) return;
...
(async () => {
  ...
  const outputPath = path.join(__dirname, 'analysis-44.json');
