...
async function analyzeEsjangStructure() {
  console.log('🔍 Starting ESJang Mall structure analysis...');
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(outputDir, 'esjang-homepage.html'),
      response.data,
      'utf-8'
    );
    ...
    const analysis = {
      ...
      productLinks: {
        ...
        patterns: {
          ...
          'view.php': $('a[href*="view.php"]').length,
          'mall': $('a[href*="mall"]').length
        }
      },
    ...
    // Look for ESJang/Icheon-specific keywords
    const pageText = $.text();
    const esjangKeywords = ['이천', '쌀', '도자기', '특산품', '농산물', '전통시장', '재래시장', '이천시장'];
    esjangKeywords.forEach(keyword => {
    ...
    // Save analysis
    const analysisPath = path.join(outputDir, 'esjang-analysis.json');
    fs.writeFileSync(analysisPath, JSON.stringify(analysis, null, 2), 'utf-8');
    
    // Console output
    console.log('\n📊 ESJang Mall Analysis Results:');
    ...
    console.error('❌ Error analyzing ESJang Mall:', error);
    throw error;
  }
}

// Run analysis
analyzeEsjangStructure()
  .then(() => {
    console.log('🎉 ESJang Mall analysis completed successfully!');
  })
  .catch((error) => {
