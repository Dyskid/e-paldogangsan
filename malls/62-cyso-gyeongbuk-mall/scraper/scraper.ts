...
class Mall62Scraper {
  private baseUrl = 'https://www.cyso.co.kr/';
  private mallId = 62;
  private mallName = 'cyso-gyeongbuk-mall';
  ...
  private extractProducts($: cheerio.CheerioAPI, categoryName: string): number {
    ...
    for (const selector of productSelectors) {
      $(selector).each((i, el) => {
        ...
        const name = $product.find('.goods-name, .product-name, h3.title').text().trim();
        const price = $product.find('.price, .goods-price, .detail-price').text().trim();
...
// Run the scraper
const scraper = new Mall62Scraper();
