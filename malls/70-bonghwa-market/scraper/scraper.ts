...
class Mall70Scraper {
  private baseUrl = 'https://bmall.cyso.co.kr/';
  private mallId = 70;
  private mallName = 'bonghwa-market';
  ...
  private extractProducts($: cheerio.CheerioAPI, categoryName: string): number {
    ...
    for (const selector of productSelectors) {
      $(selector).each((i, el) => {
        ...
        const name = $product.find('.goods-header h3, .goods-name-detail').text().trim();
        const price = $product.find('.goods-price-detail .price, .real-price').text().trim();
...
// Run the scraper
const scraper = new Mall70Scraper();
