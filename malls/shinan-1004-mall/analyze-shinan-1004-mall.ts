...
async function analyzeShinan1004Structure() {
  try {
    console.log('🔍 Starting Shinan 1004 Mall structure analysis...');
    ...
    fs.writeFileSync(
      path.join(outputDir, 'shinan1004-homepage.html'),
      response.data
    );
    ...
    const productContainerSelectors = [
      ...
      '.shop_item', '.store_item', '.mall_item'
    ];
    ...
    const categorySelectors = [
      ...
      '.product-category a', '.shop-menu a'
    ];

    categorySelectors.forEach(selector => {
      const links = $(selector);
      if (links.length > 0 && links.length < 30) { // Reasonable number of categories
    ...
    $('a[href]').each((i, el) => {
      ...
      if (href && (
        ...
        analysis.categoryAnalysis.push({
          ...
          href: fullUrl,
          isInternal: href.startsWith('/') || href.includes('shinan1004mall.kr')
        });
      }
    });
    ...
    // Save analysis
    const analysisPath = path.join(outputDir, 'shinan1004-structure-analysis.json');
    ...
    fs.writeFileSync(
      path.join(outputDir, 'shinan1004-analysis-error.json'),
