...
async function analyzeMall(): Promise<MallAnalysis> {
  const mallId = 46;
  const mallName = '여수몰';
  const mallUrl = 'http://www.yeosumall.co.kr/';
  ...
  try {
    ...
    $('.gnb-menu > li, .nav-menu > li, .category-list > li, #gnb > li, .menu > li, #category_menu > li').each((i, elem) => {
      const $elem = $(elem);
      ...
      if (name && url) {
        const category = {
          name,
          url: url.startsWith('http') ? url : `http://www.yeosumall.co.kr${url}`,
          subcategories: [] as { name: string; url: string }[]
        };
        
        // Check for subcategories
        $elem.find('.sub-menu > li > a, .dropdown-menu > li > a, ul > li > a, .submenu > li > a').each((j, subElem) => {
          const subName = $(subElem).text().trim();
          const subUrl = $(subElem).attr('href');
          if (subName && subUrl) {
            category.subcategories!.push({
              name: subName,
              url: subUrl.startsWith('http') ? subUrl : `http://www.yeosumall.co.kr${subUrl}`
            });
          }
        });
    ...
    for (const selector of listSelectors) {
      if ($(selector).length > 0) {
        ...
        // Find item selector
        const itemSelectors = ['li', '.item', '.product', '.goods', '.prd-item', '.goods_item', '.prd_item'];
        for (const itemSel of itemSelectors) {
          if ($(`${selector} ${itemSel}`).length > 0) {
            ...
            // Name selectors
            const nameSelectors = ['.name', '.title', '.prd-name', '.item-name', '.product-name', 'h3', 'h4', '.description strong', '.goods_name', '.prd_name'];
            ...
            // Price selectors
            const priceSelectors = ['.price', '.cost', '.prd-price', '.item-price', '.product-price', '.xans-product-price', '.goods_price', '.prd_price'];
    ...
    if ($('.pagination, .paging, .page-nav, .xans-product-normalpaging, .paginate, .page_num').length > 0) {
      pagination = {
        type: 'page',
        selector: '.pagination, .paging, .page-nav, .xans-product-normalpaging, .paginate, .page_num',
    ...
    const urlPatterns = {
      category: '/shop/goods_list.php?category={categoryId}',
      product: '/shop/goods_view.php?index_no={productId}',
      search: '/shop/search.php?q={keyword}'
    };
    ...
    fs.writeFileSync(
      path.join(__dirname, 'analysis-46.json'),
