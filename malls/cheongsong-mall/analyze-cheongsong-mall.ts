...
async function analyzeCsmallStructure() {
  const baseUrl = 'https://csmall.cyso.co.kr';
  
  console.log('Analyzing 칠곡몰 structure...');
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(outputDir, 'csmall-homepage.html'),
      response.data
    );
    ...
    const analysis: any = {
      baseUrl,
      mallName: '칠곡몰',
      platform: 'CYSO (Same as 사이소/상주몰/청도몰/영주장날)',
    ...
    if (productLinks.length > 0) {
      try {
        ...
        fs.writeFileSync(
          path.join(outputDir, 'csmall-product-sample.html'),
    ...
    // Save analysis
    fs.writeFileSync(
      path.join(outputDir, 'csmall-analysis.json'),
      JSON.stringify(analysis, null, 2)
    );
    
    console.log('\n=== 칠곡몰 Structure Analysis ===');
    ...
    console.log('\nAnalysis saved to:', path.join(outputDir, 'csmall-analysis.json'));
...
// Run the analysis
analyzeCsmallStructure().catch(console.error);