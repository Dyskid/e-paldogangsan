...
class Mall65Scraper {
  private baseUrl = 'https://yjmarket.cyso.co.kr/';
  private mallId = 65;
  private mallName = 'yeongju-market-day';
  ...
  private extractProducts($: cheerio.CheerioAPI, categoryName: string): number {
    ...
    for (const selector of productSelectors) {
      $(selector).each((i, el) => {
        ...
        const name = $product.find('.goods-name, .item-tit h3').text().trim();
        const price = $product.find('.goods-price .price, .detail-price strong').text().trim();
...
// Run the scraper
const scraper = new Mall65Scraper();
