...
async function analyzeEnamhaeStructure() {
  const baseUrl = 'https://enamhae.co.kr';
  
  console.log('Starting analysis of 남해몰 structure...');
  
  const outputDir = path.join(__dirname, '../../output/enamhae');
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'enamhae-homepage.html'), response.data);
    ...
    $('a[href]').each((index, element) => {
      ...
      if (href && text) {
        ...
        if (href.includes('category') || href.includes('list') || 
            ...
            text.includes('해산물') || text.includes('남해')) {
    ...
    // Add specific notes for 남해몰
    analysis.notes.push('남해군 특산물 전문몰');
    analysis.notes.push('마늘, 시금치, 해산물 특화');
    analysis.notes.push('남해 청정 농수산물 판매');

    // Save analysis results
    const outputFile = path.join(outputDir, 'enamhae-structure-analysis.json');
    ...
    console.error('Error analyzing 남해몰 structure:', error);
    throw error;
  }
}

if (require.main === module) {
  analyzeEnamhaeStructure().catch(console.error);
}

export { analyzeEnamhaeStructure };