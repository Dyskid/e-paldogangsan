/**
 * Yeongwol Mall Structure Analysis
...
async function analyzeYeongwolMall(): Promise<void> {
  console.log('🔍 Starting Yeongwol Mall structure analysis...');
  ...
  try {
    ...
    await fs.writeFile(path.join(outputDir, 'yeongwol-homepage.html'), homeResponse.data);
    ...
    for (const pathUrl of commonPaths) {
      try {
        console.log(`🔗 Trying: https://yeongwol-mall.com${pathUrl}`);
        const response = await axios.get(`https://yeongwol-mall.com${pathUrl}`, { 
          headers, 
          timeout: 20000,
        ...
        if (response.status === 200) {
          await fs.writeFile(path.join(outputDir, `yeongwol-${pathUrl.replace(/\//g, '-')}.html`), response.data);
          ...
          for (const selector of productSelectors) {
            ...
            if (products.length > 2) {
              ...
              products.slice(0, 5).each((index, element) => {
                ...
                if (name) {
                  analysis.sampleProducts.push({
                    name,
                    price,
                    image: imgSrc.startsWith('http') ? imgSrc : `https://yeongwol-mall.com${imgSrc}`,
                    link: link.startsWith('http') ? link : `https://yeongwol-mall.com${link}`
                  });
                }
              });
              ...
              analysis.categories.push({
                ...
                url: `https://yeongwol-mall.com${pathUrl}`,
    ...
    if (!foundProductPage) {
      ...
      if (productLinks.length > 0) {
        ...
        try {
          ...
          await fs.writeFile(path.join(outputDir, 'yeongwol-product-sample.html'), productResponse.data);
          ...
          if (productName) {
            analysis.sampleProducts.push({
              name: productName,
              price: productPrice,
              image: productImage.startsWith('http') ? productImage : `https://yeongwol-mall.com${productImage}`,
              link: productUrl
            });
    ...
    // Save results
    const analysisFile = path.join(outputDir, 'yeongwol-analysis.json');
    ...
    const analysisFile = path.join(__dirname, 'output', 'yeongwol-analysis.json');
...
if (require.main === module) {
  analyzeYeongwolMall().catch(console.error);
}

export { analyzeYeongwolMall };